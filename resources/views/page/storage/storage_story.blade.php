                        ...
                        @if(!$product->pivot->is_deleted && !$product->pivot->is_created)
                            @if($product->pivot->is_decreased)
                                @if($story['to'] !== null)
                                    ...
                                    @else
                                    <span class="d-block text-danger">Кол-во списано: {{$product->pivot->count}}.</span>
                                    <span class="d-block">Осталось на складе: {{$product->pivot->remainder}}.</span>
                                @endif
                            @endif
                            ...
                            @elseif($product->pivot->is_deleted && !$product->pivot->is_created)
                                @if($story['to'] !== null)
                                    ...
                                    <span class="d-block">Кол-во переместилось: {{$product->pivot->count}}.</span>
                                    @else
                                    <span class="d-block">Полностью списан.</span>
                                    <span class="d-block">Кол-во списано: {{$product->pivot->remainder}}.</span>
                                @endif
                            @elseif(!$product->pivot->is_deleted && $product->pivot->is_created)
